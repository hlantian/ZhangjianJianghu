...
sub edit {
   ...
   $inawards=("$tinuseradd1:$tinuseradd2:$tinuseradd3:$tinuseradd4:$tinuseradd5:$tinuseradd6");
    ...
    elsif ($inrating < -5) { $inmembercode = "banned"; }
    if ($inmembercode eq "banned") { $inrating = -6; }
...
$inawards = $awards;
	...
	&checkmemfile($memberfiletitle,$namenumber);
        ...
        open(FILE, ">$filetomake");
        flock(FILE, 2) if ($OS_USED eq "Unix");
        print FILE "$membername\t$innewpassword\t$inmembertitle\t$inmembercode\t$innumberofposts|$innumberofreplys\t$inemailaddress\t$showemail\t$ipaddress\t$inhomepage\t$inoicqnumber\t$inicqnumber\t$inlocation\t$interests\t$injoineddate\t$lastpostdate\t$newsignature\t$intimedifference\t$allowedforums2\t$useravatar\t$inuserflag\t$inuserxz\t$inusersx\t$personalavatar\t$personalwidth\t$personalheight\t$inrating\t$lastgone\t$invisitno\t$inaddjy\t$inmeili\t$inmymoney\t$inpostdel\t$insex\t$ineducation\t$inmarry\t$inwork\t$inborn\t$chatlevel\t$chattime\t$injhmp\t$jhcount\t$ebankdata\t$onlinetime\t$userquestion\t$inawards\t$useradd7\t$userface\t$soccerdata\t$useradd5\t";
        close(FILE);
...
if($oldmembercode ne "smo"){ 
    ...
    my $memteam1 = qq~<option value="rz1">$defrz1~ if ($defrz1 ne "");
    my $memteam2 = qq~<option value="rz2">$defrz2~ if ($defrz2 ne "");
    my $memteam3 = qq~<option value="rz3">$defrz3~ if ($defrz3 ne "");
    my $memteam4 = qq~<option value="rz4">$defrz4~ if ($defrz4 ne "");
    my $memteam5 = qq~<option value="rz5">$defrz5~ if ($defrz5 ne "");
...
$flaghtml =~ s/value=\"$userflag\"/value=\"$userflag\" selected/;
  ...
  if ($oldmembercode eq "ad") {
    ...
    <td bgcolor=#FFFFFF><font color=#333333><b>额外经验：</b></td>
    <td bgcolor=#FFFFFF><input type=text name="addjy" value="$addjy" maxlength=12 size=12></td>
    </tr><tr>
    <td bgcolor=#FFFFFF><font color=#333333><b>额外魅力：</b></td>
    <td bgcolor=#FFFFFF><input type=text name="meili" value="$meili" maxlength=12 size=12></td>
    </tr><tr>
...
####################################################
	...
	&checkmemfile($inmembername,$namenumber);
        ...
        unlink ("${lbdir}memfav/$inmembername.cgi");
        unlink ("${lbdir}memfav/open/$inmembername.cgi");
        unlink ("${lbdir}memfav/close/$inmembername.cgi");
