...
$versionnumber = "<b>L<font color=#F26522>eo</font>B<font color=#00AEEF>BS</font></b> X Build040601";
...
($memdir,$msgdir,$usrdir) = split (/\|/, getdir());
...
sub ipbanned {
    ...
    $trueipaddress = $ENV{"HTTP_X_FORWARDED_FOR"} if ($trueipaddress eq "" || $trueipaddress eq "unknown" || $trueipaddress =~ m/^192\.168\./ || $trueipaddress =~ m/^10\./);
    ...
    if ((-e "${lbdir}cache/id/$infilemembername.cgi")&&((-M "${lbdir}cache/id/$infilemembername.cgi") *86400 < 600)) {
    	#直接通过
    }
...
sub title {
    ...
    $output = qq~
<style>
.menuskin{width:88px;position:absolute;background-color:$menubackground;border:1px solid $titleborder;line-height:20px;z-index:100;visibility:hidden;}
#mouseoverstyle{BACKGROUND-COLOR: #C9D5E7; margin:1px;}
#mouseoverstyle a{color:white;}
.menuitems{margin:1px;padding:1px;word-break:keep-all;}
</style>
<script>
linkset[1]='<div class=menuitems>&nbsp;<span style=cursor:hand onClick=javascript:openScript("messanger.cgi?action=new",420,320) title=发送悄悄话短讯息给站内的朋友>发短讯给朋友</span></div><div class=menuitems>&nbsp;<a href=profile.cgi?action=modify title=编辑修改您的个人资料><font color=$menufontcolor>修改我的资料</font></a></div><div class=menuitems>&nbsp;<a href=fav.cgi?action=show&member=$inmembernames title=对您收藏的帖子进行管理><font color=$menufontcolor>管理我的收藏</font></a></div><div class=menuitems>&nbsp;<span style=cursor:hand onClick=javascript:openScript("friendlist.cgi",420,320) title=查看、添加、删除你的好友名单>管理好友列表</span></div><div class=menuitems>&nbsp;<span style=cursor:hand onClick=javascript:openScript("recopr.cgi?action=new",420,320) title=显示论坛最新的帖子列表>论坛最新帖子</span></div><div class=menuitems>&nbsp;<span style=cursor:hand onClick=javascript:openScript("recopr.cgi?action=post",420,320) title=我发表的、而且已经被别人回复过的主题列表>被回复的主题</span></div><div class=menuitems>&nbsp;<span style=cursor:hand onClick=javascript:openScript("recopr.cgi?action=reply",420,320) title=我发表回复过的主题列表>我参与的主题</span></div><div class=menuitems>&nbsp;<a href=delmycache.cgi title=清除我的缓存，确保个人所有资料都是最新的><font color=$menufontcolor>更新我的缓存</font></a></div>'
</script>
$pluginadd$headmark~;
...
sub getoneforum {
    ...
    $inmembmod = $forummodnamestemp =~ /$tempinmembername/i || (($membercode eq "cmo" || $membercode eq "mo" || $membercode eq "amo") && ($forummodnamestemp =~ /,全体版主,/ || $forummodnamestemp =~ /,全体斑竹,/)) ? "yes" : "no";
    return;
}
...
sub moderator {
    ...
    if (-e "${lbdir}cache/forums$inforum.pl") {
        ...
        if ($forummodnamestemp =~ /\,$inmembername\,/i || (($membercode eq "cmo" || $membercode eq "mo" || $membercode eq "amo") && ($forummodnamestemp =~ /,全体版主,/ || $forummodnamestemp =~ /,全体斑竹,/))) { $inmembmod = "yes"; } else { $inmembmod = "no"; }
...
sub whosonline {
    ...
    if (($thisprog ne "forums.cgi") && ($thisprog ne "leobbs.cgi") && ($thisprog ne "loginout.cgi")) {
    	my $tempusername1 = $tempusername;
    	$tempusername1 =~ s/\(/\\\(/isg;
    	$tempusername1 =~ s/\)/\\\)/isg;
    	$tempusername1 =~ s/\|/\\\|/isg;
        if ($onlinedata1 =~ /(^|\n)$tempusername1\t/i) {
	    $onlinedata1 =~ s/(^|\n)$tempusername1\t(.*?)\t.*?\t.*?\t(.*?)\t(.*?)\t(.*?)\t.*?\t/$1$tempusername\t$2\t$currenttime\t$where\t$3\t$4\t$5\t$where2\t/i;
	    $onlinetimeadd = $currenttime - $2 if (($thisprog eq "post.cgi")||($thisprog eq "poll.cgi")||($thisprog eq "messanger.cgi"));
	    $memberprinted = "yes";
	}
	@onlinedata=split(/\n/,$onlinedata1);
	$onlinedatanumber=@onlinedata;
	if (-e "${lbdir}cache/onlineuserlist.pl") {
	    eval{ require "${lbdir}cache/onlineuserlist.pl"; };
	    if ($@) { unlink ("${lbdir}cache/onlineuserlist.pl");}
	}
    } else {
        @onlinedata1=split(/\n/,$onlinedata1);
        ...
        if ((($onlineuserlist ne "_")||($onlineuserlisthidden ne "_"))&&((-M "${lbdir}cache/onlineuserlist.pl") *86400 >= 300)||(!(-e "${lbdir}cache/onlineuserlist.pl"))) {
            open (FILE, ">${lbdir}cache/onlineuserlist.pl");
            print FILE qq~\$onlineuserlist = qq($onlineuserlist);\n~;
            print FILE qq~\$onlineuserlisthidden = qq($onlineuserlisthidden);\n~;
            print FILE qq~1;~;
            close(FILE);
        }
    }
...
sub output {
        ...
        $title =~ s/\$/\&\#036\;/sg;
	undef $/;
        open(FILE,"${lbdir}data/template.cgi");
...
if (($cpudisp eq "1")||($membercode eq "ad")||($membercode eq "smo")||($membercode eq "cmo")||($membercode eq "amo")||($membercode eq "mo")) {
    ...
    print "<center><font color=$cpudispcolor>当前页面执行消耗时间： $spenttime 毫秒　[Gzip: Off]";
}
exit;
}
...
if ($indexforum ne "no") {
    $output .= qq~<table width=$tablewidth align=center cellspacing=0 cellpadding=1 bgcolor=$navborder><tr><td><table width=100% cellspacing=0 cellpadding=3><tr><td bgcolor=$navbackground><img src=$imagesurl/images/item.gif align=absmiddle width=11> <font color=$navfontcolor><a href=leobbs.cgi>$boardname</a> → 错误： $where</td><td bgcolor=$navbackground align=right></td></tr></table></td></tr></table>~;
} else {
    $output .= qq~<table width=$tablewidth align=center cellspacing=0 cellpadding=1 bgcolor=$navborder><tr><td><table width=100% cellspacing=0 cellpadding=3><tr><td bgcolor=$navbackground><img src=$imagesurl/images/item.gif align=absmiddle width=11> <font color=$navfontcolor>错误： $where</td><td bgcolor=$navbackground align=right></td></tr></table></td></tr></table>~;
}
	$output .= qq~<p><table cellpadding=0 cellspacing=0 width=$tablewidth bgcolor=$tablebordercolor align=center><tr><td><table cellpadding=6 cellspacing=1 width=100%>~;
    }
    else { $versionnumber="msg"; }

if ($catbackpic !~ /^background\=/)  { $catbackpic = "background=$imagesurl/myimages/$catbackpic"; }
