...
if ((!$inmembername) or ($inmembername eq "客人")) { $inmembername = "客人"; $myrating= "-1"; $mymembercode="no"; &error("普通错误&客人不能查看贴子内容，请注册或登录后再试") if ($guestregistered eq "off");}
else {
...
if ($catbackpic ne "")  { $catbackpic = "background=$imagesurl/myimages/$catbackpic"; }
    ...
    chomp $topicinfo;
    $topicinfo =~ s/[\a\f\n\e\0\r]//isg;
    ($topicid, $topictitle, $topicdescription, $threadstate, $threadposts, $threadviews, $startedby, $startedpostdate, $lastposter, $lastpostdate, $posticon, $inposttemp) = split (/\t/,$topicinfo);
...
function loadThreadFollow(f_id,t_id,r_id,ftype){
...
if (targetImg.loaded=="no"){ document.frames["hiddenframe"].location.replace("getphotoinfo.cgi?forum="+f_id+"&topic="+t_id+"&reply="+r_id+"&ftype="+ftype); }
...
$output .= qq~<table cellpadding=1 cellspacing=0 width=$tablewidth align=center><tr><td height=22><span id=LeoBBSad style=display:none></span></td></tr>
<tr><form action="download.cgi" name="Download" method=POST target=_blank><input type=hidden name=forum><input type=hidden name=topic><input type=hidden name=reply></form><td align=center width=1></td>
<td width=420 valign=bottom>$newthreadbutton$newreplybutton$newpollbutton</td>
...
<td align=right>$savefile$reporticon$favicon$printpageicon$pagpageicon$bookmarkpage$sendtofriendicon $fontsizeselect&nbsp;</td></tr></table></td><td bgcolor=$tablebordercolor width=1 height=24></td></tr></table>
<table cellspacing=0 width=$tablewidth bgcolor=$tablebordercolor align=center><tr><td height=1></td></tr></table>
~;
...
if ($dispshowcount eq "yes") {
    if (open(COUNTFILE, "${lbdir}FileCount/$inforum/$inforum\_$intopic.cgi")) {
    	undef $/;
	my $filedowncount = <COUNTFILE>;   
        close(COUNTFILE); 
        $/="\n";
        @filedowncount = split (/\n/, $filedowncount);
	@filedowncount = grep(/$inforum\_$intopic/,@filedowncount);
    }
}
...
if (($useads ne "no")&&(-e "${imagesdir}ad.js")) {
    $addads = "<span id=LeoBBSad>$boardname广告位置</span>";
}
else { $addads = ""; }
  ...
  if ($usruploaddata > 0) {
	...
	if ($usruploaddatareply > 0) {
	    ...
	    my @downcount      = grep(/$inforum\_$intopic\_$rrn\./,@filedowncount);
            ...
            if ($#downcount>=0 && $dispshowcount eq "yes") {
                my $DownDetail = $downcount[0]; chomp $DownDetail;
	...
	if (($up_ext eq "gif")||($up_ext eq "jpg")||($up_ext eq "jpe")||($up_ext eq "jpeg")||($up_ext eq "tif")||($up_ext eq "png")||($up_ext eq "bmp")) {
	    ...
	    require "dobtinfo.pl";
	    	...
	    	$addme = qq(<font color=$fonthighlight>相关附件</font>：<a href="javascript:Download('$forumno','$topicno','$replyno')"><img src=$imagesurl/icon/$filetype.gif border=0 width=16 alt="该主题有一个“$filetype”类型附件，点击下载"></a> (共 $fileinfo[7] 字节)$DownTime<br><br>);
    	    } else {
...
#    next if ($membername eq "");
    ...
    if ($forummodnamestemp =~ /\,$membername{$membername}\,/i) { $inmembmod = "yes"; } else { $inmembmod = "no"; }
	...
	$post =~ s/\[USECHGFONTE\]//isg;
        $post =~ s/(^|\>)(\[这个.+?最后由.+?编辑\])(.*)/$1$2<div id=CArea$inforum$editpostnumber>$3<\/div><script>var _old$inforum$editpostnumber=CArea$inforum$editpostnumber.innerHTML;ConFONT('$inforum$editpostnumber','$imagesurl','1');<\/script>/isg;
	...
	$signature{$membername} = "";
        $post = qq(<br>------------------------<br><font color=$posternamecolor>此用户的发言已经被屏蔽！<br>如有疑问，请联系坛主！</font><br>------------------------<BR></td><td width=16>);
    }
    ...
    if ((($mymembercode eq "ad")||($mymembercode eq "mo")||($mymembercode eq 'smo')||(($myinmembmod eq "yes")&&($mymembercode ne 'amo')))&&($membercode{$membername} ne "ad")&&($membercode{$membername} ne "smo")&&($membercode{$membername} ne "cmo")&&($membercode{$membername} ne "amo")&&($membercode{$membername} ne "mo")&&($tempname ne "")&&($membername ne "客人")) { $rateuser = qq~　<a href="userrating.cgi?membername=$tempname&oldforum=$inforum&oldtopic=$intopic&oldpostno=$editpostnumber" title=给此用户投票><img src=$imagesurl/images/poll1.gif border=0 width=16 height=14>投票</a>~; } else { $rateuser=""; }
    ...
    if ($editpostnumber eq 1) { $louceng ="<b>[<FONT COLOR=$fonthighlight>楼 主</font>]"; } else { $louceng = qq~<b>[第 <FONT COLOR=$fonthighlight>$editpostnumber</FONT> 楼]~; }
...
<br><font color=$postfontcolortwo>精华贴数: <b><font color=$fonthighlight>$jhcount{$membername}</font></b> 篇
<br><font color=$postfontcolorone>注册日期: $joineddate{$membername}<BR>
</td>
<td width=1 height=100% rowspan=5 bgcolor=$titlecolor></td>
<td valign=top width=600>$pvmsggraphic{$membername}$profilegraphic{$membername}$searchgraphic{$membername}$friendgraphic{$membername}$emailgraphic{$membername}$homepagegraphic{$membername}$oicqgraphic{$membername}$icqgraphic{$membername}$copygfx$replygraphic$replynow$chgposticon</td><td align=right>$louceng</td>
</tr>
<tr><td colspan=2 height=1 bgcolor=$tablebordercolor></td></tr>
<tr><td colspan=2 valign=top>
<table cellspacing=0 width=100% style=TABLE-LAYOUT:fixed height=$vheight{$membername}>
<tr><td width=32 valign=top>$posticon&nbsp;</td><td style="WORD-WRAP:break-word;$paraspace;$wordspace\pt" valign=top><span id=post$editpostnumber style=color:$postfontcolor;font-size:${postfontsize}px>$addme$post<BR></td></tr>
<tr><td colspan=2 height=22 valign=bottom>$addads<BR></td></tr>
</table>
</td></tr>
<tr><td colspan=2 height=1 bgcolor=$tablebordercolor></td></tr>
<tr><td><font color=$postfontcolor>$editgraphic$delgraphic<img src=$imagesurl/images/posttime.gif width=16 alt=发贴时间 align=absmiddle>$postdate　<img src=$imagesurl/images/ip.gif width=13 alt="$fromproxy" align=absmiddle>IP: $postipaddress</td><td align=right valign=bottom>[本文章共 <i>$postsize</i> 字节]　<a href=#top><img src=$imagesurl/images/p_up.gif height=17 border=0 align=absmiddle title=返回顶端></a>&nbsp;</td>
...
<tr><td>&nbsp;<a href=#top><img src=$imagesurl/images/gotop.gif height=15 width=15 border=0 align=absmiddle>顶端</a>　<a href=fav.cgi?action=add&forum=$inforum&topic=$intopic><img src=$imagesurl/images/addfavorite.gif height=15 width=18 border=0 align=absmiddle>加到"个人收藏夹"</a></td><td nowrap align=right><font color=$menufontcolor><img src=$imagesurl/images/forumme.gif> <b>主题管理</B>：
...
<a href=postings.cgi?action=unlocktop&forum=$inforum&topic=$intopic>取消固顶</a> <img src=$imagesurl/images/fg.gif><BR>
...
~;
if (($useads ne "no")&&(-e "${imagesdir}ad.js")) {
    $output .= qq~<SCRIPT src=$imagesurl/ad.js></SCRIPT>~;
}
