...
#&getoneforum("$inforum");
	...
	&error("发表新投票&对不起，你的删贴率超过了<b>$deletepercent</b>%，管理员不允许你发表新投票！") if ($postdel / ($numberofposts + $numberofreplys) >= $deletepercent / 100);
    }
...
if ($canchgfont ne "no") {
    ...
    if ((($onlinetime + $onlinetimeadd) < $onlinepost)&&($onlinepost ne "")&&($membercode ne "ad")&&($membercode ne "smo")&&($membercode ne "cmo")&&($membercode ne "mo")&&($membercode ne "amo")&&($membercode !~ /^rz/))     {  $onlinetime = $onlinetime + $onlinetimeadd; &error("发表新投票&对不起，本论坛不允许在线时间少于 $onlinepost 秒的用户发表投票！你目前已经在线 $onlinetime 秒！"); }
...
if ($useemote eq "yes") {
	...
	$cleanmembername =~ tr/A-Z/a-z/;
        $numberofposts++ if ($forumallowcount ne "no");
	...
	if (!(-e "$filetoopens.lck")) {
		...
		$posts = 0 if ($posts eq "");$threads = 0 if ($threads eq "");
	        ...
	        print FILE "\$threads = $threads;\n\$posts = $posts;\n\$todayforumpost = \"$todayforumpost\";\n1;\n";
                ...
                unlink ("${lbdir}cache/forumcache.pl") if ($threads < 10);
	}
        else {
    	    unlink ("$filetoopens.lck") if ((-M "$filetoopens.lck") *86400 > 30);
	}
...
#&getoneforum("$inforum");
    ...
    if ((($onlinetime + $onlinetimeadd) < $onlinepost)&&($onlinepost ne "")&&($membercode ne "ad")&&($membercode ne "smo")&&($membercode ne "cmo")&&($membercode ne "mo")&&($membercode ne "amo")&&($membercode !~ /^rz/))     {  $onlinetime = $onlinetime + $onlinetimeadd; &error("发表新投票&对不起，本论坛不允许在线时间少于 $onlinepost 秒的用户投票！你目前已经在线 $onlinetime 秒！"); }

	&error("投票错误&你是客人无权投票！")  if (($inmembername eq "客人")||($inmembername eq ""));
        if (($membercode eq "banned")||($membercode eq "masked"))     { &error("投票错误&您被禁止发言或者发言被屏蔽，请联系坛主解决！"); }
