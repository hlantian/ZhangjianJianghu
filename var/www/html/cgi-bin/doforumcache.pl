...
open(LOCKFILE, ">${lbdir}cache/forumcache.pl.lock");
close(LOCKFILE);
...
else { unlink("${lbdir}cache/forumcache.pl.lock"); &error("论坛还没建立&请先在管理区建立分论坛！或者分类信息完全丢失，请坛主到管理区重建论坛主界面！"); }
@forums=split(/\n/,$forums);
...
#	$topictitletemp =~ s/\&/\&amp;/g;
    ...
    if ($teamlogo eq "") { $team = ""; } elsif (($teamurl eq "")||($teamurl eq "http://")) { $team=qq~$teamlogo~; } else { $team=qq~<a href=$teamurl>$teamlogo</a>~; }

    if ($category =~ /^childforum-[0-9]+/) {
    	$cforums[$forumid] = "$forumid\t$category\t\t$forumname\t";
	$topforumno=$category;
...
if ($dispborn ne "no") {
    ...
    unless (($dispborn eq "auto")&&($birthdayuser eq "")) {
        ...
        $birthdayoutput = qq~<table cellpadding=0 cellspacing=0 width=$tablewidth bgcolor=$tablebordercolor align=center><tr><td><table cellpadding=6 cellspacing=1 width=100%><tr><td bgcolor=$titlecolor colspan=7 $catbackpic><font color=$titlefontcolor><B>-=> 今天过生日的注册用户 （共 $borncount 人）</b>     [<a href="$imagesurl/images/rili.htm" target=_blank>论坛日历</a>]</td></tr><tr><td bgcolor=$forumcolorone align=center width=26><img src=$imagesurl/images/born.gif alt="今天过生日人员名单" width=16></td><td bgcolor=$forumcolortwo colspan=6 width=*><img src=$imagesurl/images/none.gif width=400 height=0><br>　<font color=$forumfontcolor>$birthdayuser</td></tr></table></td></tr></table><img src=$imagesurl/images/none.gif height=5><br>~;
    }
}

require "${lbdir}data/forumcate.pl" if (($cancmodoutput ne "no")&&(-e "${lbdir}data/forumcate.pl"));

open (FILE, ">${lbdir}cache/forumcache.pl");
