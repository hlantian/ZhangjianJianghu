...
$versionnumber = "LeoBBS X Build040601";
...
¡¡¡¡ÏÂÃæ 1 ºÍ 2 ÖĞµÄÄ¬ÈÏÉèÖÃÊÇÓÉ³ÌĞò×Ô¶¯ÅĞ¶ÏÉú³ÉµÄ£¬ÊÊÓÃÓÚ´ó²¿·Ö°²×°±¾³ÌĞòµÄ¿Í»§£¬Èç¹ûÓĞ´íÎó£¬Çë×ÔĞĞĞŞ¸Ä³ÉÕıÈ·µÄÖµ¡£<BR>
...
sub myimgdir {
  ...
  if (($html_dir eq "")||(!(-e "$html_dir/images/abstop.gif"))) {
...
sub changedirname {
    ...
    my $searchdir = $searchdir[0];
       $searchdir = $searchdir[1] if ($searchdir eq "search.cgi");
    my @memdir = grep(/^members/i, @files);
    my $memdir = $memdir[0];
    my @msgdir = grep(/^messages/i, @files);
    my $msgdir = $msgdir[0];
