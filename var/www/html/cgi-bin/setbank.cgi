...
if ($action eq "login")
{
	$inmembername = $query->param("adminname");
	$inpassword = $query->param("adminpass");
	if ($inpassword ne "") {
...
else
{
	...
	$ipaddress = $trueipaddress if ($trueipaddress ne "" && $trueipaddress ne "unknown" && $trueipaddress !~ m/^192\.168\./ && $trueipaddress !~ m/^10\./);
	&logaction($inmembername, "<b>从 $ipaddress 以密码 $inpassword 登录行长办公室失败。</b>") if ($inmembername && $inpassword);	
	&ebankadminlogin;
}
...
sub ebankadminlogin
{
	print qq~<table width=100% cellpadding=6 cellspacing=0>
<tr>
	<td bgcolor=#333333 colspan=2><font color=#ffffff><b>欢迎来到银行行长办公室</b></font></td>
</tr>
<form action=$thisprog method=POST>
...
<tr>
	...
	<td bgcolor=#ffffff valign=middle><input type=text name=verifynum size=4 maxlength=4>　　<img src=verifynum.cgi border=0 height=20 align=absmiddle></td>
</tr>
<tr>
...
sub checkverify {
	...
	if ($verifynum ne $trueverifynum || $currenttime > $verifytime + 300)
	{#验证码有效时间仅为5分钟
		$inpassword = "";
	}
	else
