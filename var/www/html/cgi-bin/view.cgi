...
if ($addmefile == 1)
{
	...
	if ($up_ext eq "gif" || $up_ext eq "jpg" || $up_ext eq "png" || $up_ext eq "bmp")
	{
		...
		{
			$addme = qq~<img src=$imagesurl/icon/$filetype.gif border=0 width=16> 此主题相关图片如下：<br><a href=attachment.cgi?forum=$inforum&topic=$intopic&postno=1&type=.$up_ext target=_blank><img src=attachment.cgi?forum=$inforum&topic=$intopic&postno=1&type=.$up_ext border=0 alt="按此在新窗口浏览图片" onload="javascript: if(this.width > screen.width - 333) this.width = screen.width - 333" onmousewheel="return bbimg(this)"></a><br><br>~;
		}
...
if ($membercode eq "masked") {
    $addme = "";
    $post = qq(<br>------------------------<br><font color=$posternamecolor>此用户的发言已经被屏蔽！<br>如有疑问，请联系坛主！</font><br>------------------------<BR>);
}
...
for ($i = 1; $i < @threads; $i++)
{
	...
	if ($up_ext eq "gif" || $up_ext eq "jpg" || $up_ext eq "png" || $up_ext eq "bmp")
	{
		...
		{
			$addme = qq~<img src=$imagesurl/icon/$filetype.gif border=0 width=16> 此主题相关图片如下：<br><a href=attachment.cgi?forum=$inforum&topic=$intopic&postno=$rnrn&type=.$up_ext target=_blank><img src=attachment.cgi?forum=$inforum&topic=$intopic&postno=$rnrn&type=.$up_ext border=0 alt="按此在新窗口浏览图片" onload="javascript: if(this.width > screen.width - 333) this.width = screen.width - 333" onmousewheel="return bbimg(this)"></a><br><br>~;
		}
