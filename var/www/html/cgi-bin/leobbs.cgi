...
if ($catsbackpicinfo ne "")  { $catsbackpicinfo = "background=$imagesurl/myimages/$catsbackpicinfo"; }

if (((-e "${lbdir}cache/forumcache.pl")&&((-M "${lbdir}cache/forumcache.pl") *86400 < 180))||(-e "${lbdir}cache/forumcache.pl.lock")) {
    unlink("${lbdir}cache/forumcache.pl.lock") if ((-M "${lbdir}cache/forumcache.pl.lock") *86400 > 15);
    if (-e "${lbdir}cache/forumcache.pl.lock") {
    	select(undef, undef, undef, 0.2);
	select(undef, undef, undef, 0.2) if (-e "${lbdir}cache/forumcache.pl.lock");
	select(undef, undef, undef, 0.2) if (-e "${lbdir}cache/forumcache.pl.lock");
	select(undef, undef, undef, 0.3) if (-e "${lbdir}cache/forumcache.pl.lock");
	unlink("${lbdir}cache/forumcache.pl.lock");
	&error("普通错误&服务器忙，请稍后再试！！");
    }
    if (-e "${lbdir}cache/forumcache.pl") {
        eval{ require "${lbdir}cache/forumcache.pl"; };
        if ($@) { unlink ("${lbdir}cache/forumcache.pl"); require "doforumcache.pl"; }
        $forumcached = "yes";
...
<td width=64 align=right><a href=team.cgi target=_blank><img src=$imagesurl/images/team.gif width=57 height=17 border=0 alt=显示管理团队 align=absmiddle></a></td>
<td width=64 align=right><a href=memberlist.cgi?a=4 target=_blank><img src=$imagesurl/images/userlist.gif width=57 height=17 border=0 alt=显示用户列表 align=absmiddle></a></td>
<td width=64 align=right><a href=memberlist.cgi?a=1 target=_blank><img src=$imagesurl/images/top.gif width=57 height=17 border=0 alt=显示发帖量排名 align=absmiddle></a></td>
</tr></table>
<style>
...
if ($inmembername eq "客人") {
  ...
  if ((-e "${lbdir}cache/myinfo/$memberfilename.pl")&&((-M "${lbdir}cache/myinfo/$memberfilename.pl") *86400 < 900)) {
...
</td></tr></form></table></td></tr></table><img src=$imagesurl/images/none.gif height=7><br>
...
foreach (@rearrangedforums) {
    ...
    next unless (($hiddenforum eq "no")||($membercode eq "ad")||($membercode eq "smo")||($membercode eq "cmo")||($membercode eq "mo"));
...
#    $forumdescription  = &HTML("$forumdescription");
	...
	$output .= qq~<BR><table cellspacing=0 width=$tablewidth bgcolor=$tablebordercolor align=center><tr><td height=1></td></tr></table><table cellpadding=6 cellspacing=0 width=$tablewidth height=24 align=center bordercolor=$tablebordercolor border=1>~;
        if (!$catshow{$categoryplace}) {
      	    $output .= qq~<tr><td bgcolor=$catback $catsbackpicinfo height=24><table width=100% cellpadding=0 cellspacing=0><tr><td><a href=$thisprog?catlog=$categoryplace title=收缩此分类><img src=$imagesurl/images/cat1.gif border=0 width=9> <font color=$catfontcolor><b>$category</a></td><td align=right width=150>$cmodoutput[$categoryplace]</td></tr></table></td></tr></table>~;
      	}
      	else {
      	    $output .= qq~<tr><td bgcolor=$catback $catsbackpicinfo height=24><table width=100% cellpadding=0 cellspacing=0><tr><td><a href=$thisprog?catlog=$categoryplace title=展开此分类><img src=$imagesurl/images/cat.gif border=0 width=9> <font color=$catfontcolor><b>$category</a></td><td align=right width=150>$cmodoutput[$categoryplace]</td></tr></table></td></tr></table>~;
      	}
    }
    ...
    if (!$catshow{$categoryplace}) {
	    if (($dispchildforumnum ne "no")&&($#forums >= 0)) {
	        ...
	        $childforumnums = $childforumnums > 0 ? qq~　<span title="有 $childforumnums 个下属子论坛">[$childforumnums]</span>~ : "";
	    }
    	    $forumnameadd = "$boardname - $forumname";
	    $forumnameadd =~ s/\&\#039\;/\\\'/g;
	    $titleinfos{"$forumname\n"} =~ s/\|/\n/isg;
	    my $forumbookmark = qq~<span style=CURSOR:hand onClick="window.external.AddFavorite('$boardurl/forums.cgi?forum=$forumid', '$forumnameadd')"><IMG SRC=$imagesurl/images/fav_add.gif width=15 ALT="将 $forumname 添加到收藏夹"></span>~;
		...
		if ($privateforum eq "yes") { $folderpicture = qq~<img src=$imagesurl/images/$bm_havenew width=13 style=cursor:hand onClick=javascript:O3($forumid)>~; }
		      elsif ($startnewthreads eq "follow") { $folderpicture = qq~<img src=$imagesurl/images/$pl_havenew width=13 style=cursor:hand onClick=javascript:O3($forumid)>~; }
		      elsif ($startnewthreads eq "yes")    { $folderpicture = qq~<img src=$imagesurl/images/$zg_havenew width=13 style=cursor:hand onClick=javascript:O3($forumid)>~; }
		      elsif ($startnewthreads eq "all")    { $folderpicture = qq~<img src=$imagesurl/images/$kf_havenew width=13 style=cursor:hand onClick=javascript:O3($forumid)>~; }
		      elsif ($startnewthreads eq "cert")   { $folderpicture = qq~<img src=$imagesurl/images/$rz_havenew width=13 style=cursor:hand onClick=javascript:O3($forumid)>~; }
        	$posts   = qq~<font color=$fonthighlight><b>$posts</b>~;
		$threads = qq~<font color=$fonthighlight><b>$threads</b>~;
		$todayforumpost = qq~<font color=$fonthighlight><b>$todayforumpost</b>~;
	    }
    	    else {
        	if ($privateforum eq "yes") { $folderpicture = qq~<img src=$imagesurl/images/$bm_nonew width=13 style=cursor:hand onClick=javascript:O3($forumid)>~; }
		      elsif ($startnewthreads eq "follow") { $folderpicture = qq~<img src=$imagesurl/images/$pl_nonew width=13 style=cursor:hand onClick=javascript:O3($forumid)>~; }
		      elsif ($startnewthreads eq "yes")    { $folderpicture = qq~<img src=$imagesurl/images/$zg_nonew width=13 style=cursor:hand onClick=javascript:O3($forumid)>~; }
		      elsif ($startnewthreads eq "all")    { $folderpicture = qq~<img src=$imagesurl/images/$kf_nonew width=13 style=cursor:hand onClick=javascript:O3($forumid)>~; }
		      elsif ($startnewthreads eq "cert")   { $folderpicture = qq~<img src=$imagesurl/images/$rz_nonew width=13 style=cursor:hand onClick=javascript:O3($forumid)>~; }
        	$posts   = qq~<font color=$forumfontcolor>$posts</b>~;
		$threads = qq~<font color=$forumfontcolor>$threads</b>~;
		$todayforumpost = qq~<font color=$fonthighlight>$todayforumpost</b>~;
	    }
	    if ($startnewthreads eq "no") { $folderpicture = qq~<img src=$imagesurl/images/$jh_pic width=13 style=cursor:hand onClick=javascript:O3($forumid)>~; }
	...
	if ($forumnamedisp eq 1 && $startnewthreads ne "onlysub") { $forumnamelink = qq~<table width=100% cellpadding=2 cellspacing=0><tr><td width=93%>$forumname$hiddeninfo</td><td width=12><a href=post.cgi?action=new&forum=$forumid><img src="$imagesurl/images/fpost2.gif" alt="在此分论坛发表新主题" width=12 border=0></a></td><td width=12><a href=poll.cgi?action=new&forum=$forumid><img src="$imagesurl/images/fpost1.gif" alt="在此分论坛发表新投票" width=12 border=0></a></td><td width=12><a href=jinghua.cgi?action=list&forum=$forumid><img src="$imagesurl/images/fpost3.gif" alt="查看此分论坛的精华帖子" width=12 border=0></a></td></tr><tr><td colspan=4 width=100%><img src=$imagesurl/images/forumme.gif width=9> <font color=$forumfontcolor>$forumdescription</td></tr></table></td></tr></table>~; }
...
<table width=100% cellpadding=1><tr><td width=3%>$team</td><td width=1></td><td width=97%>$forumnamelink</td><td bgcolor=$forumcolorone align=center width=90><font color=$forumfontcolor>$modout[$forumid]</td>
...
if (-e "${lbdir}data/unionoutput.pl") {
    $output .= qq~<table cellpadding=0 cellspacing=0 width=$tablewidth bgcolor=$tablebordercolor align=center><tr><td><table cellpadding=6 cellspacing=1 width=100%>~;
    require "${lbdir}data/unionoutput.pl";
    $output .= qq~</table></td></tr></table><img src=$imagesurl/images/none.gif height=5><br>~;
}
...
<tr><td bgcolor=$forumcolorone align=center width=26><img src=$imagesurl/images/online.gif width=16 $membergone></td>
...
if (($displink eq "yes")&&($links ne "")) {
    $links = &HTML("$links");
    $output .= qq~</td></tr></table></td></tr></table><img src=$imagesurl/images/none.gif height=5><br>
...
<tr><td>　现在时间是 $timenow ($basetimes)</td>
...
</table><center><br><br>$adfoot</center><br>~;
