...
#        $post =~s/\[post=(.+?)\](.+?)\[\/post\]/(本内容已被隐藏)/isg; 
	...
	if ($post=~/LBSALE\[(.*?)\]LBSALE/sg) {
		...
		$isbuyer="yes" if ($allbuyer =~ /\t$inmembername\t/i);
            ...
            unless (($inmembername eq $membername )||($mymembercode eq "ad")||($mymembercode eq 'smo')||($mymembercode eq 'mo')||($mymembercode eq 'amo')||($myinmembmod eq "yes")||($isbuyer eq "yes")) {
	    	...
	    	$buyeroutput = "";
	    	if (($inmembername eq $membername )||($mymembercode eq "ad")||($mymembercode eq 'smo')||($mymembercode eq 'mo')||($mymembercode eq 'amo')||($myinmembmod eq "yes")) {
                    if ($allbuyerno > 0 ) {
