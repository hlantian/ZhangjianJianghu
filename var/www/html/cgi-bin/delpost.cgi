		...
		unless ($cleartoedit eq "yes") { $cleartoedit = "no"; }
		    ...
		    &checkmemfile($nametocheck,$namenumber);
	  		...
	  		if (($membername ne "")&&($password ne "")) {#7
	    		    ...
	    		    $lastgone=time;
	    		    $postdel ++ if ($inleavemessage ne "no");
...
sub deletepost {
  ...
  if ($checked eq "yes"){ #1
    ...
    unless ($cleartoedit eq "yes") { $cleartoedit = "no"; }
        ...
        if ($cleartoedit eq "yes" && ($checkcandel == $delcount || $checkcandelm eq "YES")) {
	  	...
	  	foreach $posttodelete (@inpostno){
		    ...
		    $postdelnum ++ if ($inleavemessage eq "yes");
		}
			$nametocheck = $startedby;
			$nametocheck =~ s/ /\_/g;
			$nametocheck =~ tr/A-Z/a-z/;
	...
	($topicno1,$topictitle1,$topicdescription,$threadstate,$threadposts,$threadviews,$startedby,$startedpostdate,$postermembername1,$postdate1,$posticon,$inposttemp1) = split(/\t/,$linedata);
	$threadposts -= $delcount;
	$newline1="$topicno1\t$topictitle1\t$topicdescription\t$threadstate\t$threadposts\t$threadviews\t$startedby\t$startedpostdate\t$postermembername2\t$postdate2\t$posticon\t$inposttemp\t";
        if (open(FILE, ">$filetoopen")) {
            flock(FILE, 2) if ($OS_USED eq "Unix");
            print FILE "$newline1\n";
            close(FILE);
	...
	unlink ("${lbdir}FileCount/$inforum/$inforum\_$intopic.pl");
       ...
       foreach $pno (@oldinpostno) {
       	  ...
       	  for(my $i=$pno+1;$i<$tt;$i++){
       	      $ii=$i-1;
       	      rename ("${lbdir}sale/$inforum\_$intopic\_$i.cgi","${lbdir}sale/$inforum\_$intopic\_$ii.cgi") if (-e "${lbdir}sale/$inforum\_$intopic\_$i.cgi");
       	  }
       }
