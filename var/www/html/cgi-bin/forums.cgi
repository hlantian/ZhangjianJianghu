...
<tr><td width=2></td><td align=center width=34>$announcetemp1</td><td width=*>$announcedisp</td><td align=right width=400><p>
<SCRIPT>
...
<form action=forums.cgi method=post name=jump1><A href=javascript:JM_setTarget() value='' id=globalTarget><img src=$imagesurl/images/$wlogo height=21 border=0 alt=查看帖子使用的模式？ align=absmiddle></a>　
...
if (($membercode eq "ad")||($membercode eq 'smo')||($inmembmod eq "yes")) {
    ...
    $admini = qq~<img src=$imagesurl/images/icon.gif width=14> <b><span style=cursor:hand onClick=javascript:showadmini()><font color=$fonthighlight id=admini0 title=设置是否显示版主快速操作栏>$admini版主操作</font></span><span id=admini></span></b>　~;
    $multimanage  = qq~<td bgcolor=$titlecolor width=27 align=center $catbackpic><font color=$titlefontcolor><b>选</b></td>~;
...
if (($privateforum eq "yes")||($xzbopen eq "no")||($startnewthreads eq "no")||($startnewthreads eq "cert")) { $newthreadbutton3 = ""; }
  else { $newthreadbutton3 = qq~<a href=xzb.cgi?action=new&forum=$inforum><img src=$imagesurl/images/$newxzblogo border=0 alt=张帖一个小字报 width=99 height=25></a>　~; }
...
<table cellpadding=0 cellspacing=0 width=$tablewidth align=center><tr><td>$admini$xzb</td><td align=right width=*>$jinghua$forumlog$lookstyles</td><td width=4></td></tr></table>
...
<table cellpadding=0 cellspacing=0 width=$tablewidth height=27 bordercolor=$tablebordercolor border=1>
<tr><td bgcolor=$titlecolor $catbackpic width=32 align=center $catbackpic><font color=$titlefontcolor><b>状态</b></td>
<td bgcolor=$titlecolor $catbackpic width=* align=center $catbackpic><font color=$titlefontcolor><b>主　题</b> (点心情符为新闻方式阅读)</td>
<td bgcolor=$titlecolor $catbackpic align=center width=80 $catbackpic><font color=$titlefontcolor><b>作 者</b></td>
<td bgcolor=$titlecolor $catbackpic align=center width=32 $catbackpic><font color=$titlefontcolor><b>回复</b></td>
<td bgcolor=$titlecolor $catbackpic align=center width=32 $catbackpic><font color=$titlefontcolor><b>点击</b></td>
<td bgcolor=$titlecolor $catbackpic width=195 align=center $catbackpic><font color=$titlefontcolor><b>　 最后更新 　 | 最后回复人</b></td>
...
$jnum = 0;
$fnum = 0;
if (($screenmode >=8)||($tablewidth > 770)) { $topictitlemax = 71; } else { $topictitlemax = 40; }
...
foreach $topic (@toptopic) {
    ...
    if ((lc($inmembername) eq lc($startedby))&&($nodispown eq "yes")){ $mypost="<img src=$imagesurl/images/$mypost_blogo title=我发表的主题> "; $addonlength +=2; } else { $mypost=""; }

    $topicicon = "<img src=$imagesurl/images/topicnonew.gif width=14 border=0>";

    if ($inmembername ne "客人") {
        if (($threadposts >= $hottopicmark) && ($forumlastvisit < $lastpostdate))    { $topicicon = "<img src=$imagesurl/images/topichot3.gif width=14 border=0>"; }
        elsif (($threadposts >= $hottopicmark) && ($forumlastvisit > $lastpostdate)) { $topicicon = "<img src=$imagesurl/images/topichotnonew.gif width=14 border=0>"; }
        elsif (($threadposts <  $hottopicmark) && ($forumlastvisit < $lastpostdate)) { $topicicon = "<img src=$imagesurl/images/topicnew3.gif width=14 border=0>"; }
        elsif (($threadposts <  $hottopicmark) && ($forumlastvisit > $lastpostdate)) { $topicicon = "<img src=$imagesurl/images/topicnonew.gif width=14 border=0>"; }
    }
    ...
    if ($threadstate eq "closed") { $topicicon = "<img src=$imagesurl/images/topiclocked3.gif width=14 border=0>"; }
    ...
    $output .=qq~<table cellspacing=0 width=$tablewidth bordercolor=$tablebordercolor border=1 height=32><tr><td align=center width=30 bgcolor=$forumcolorone>$topicicontemp</td>
...
$output .= qq~</tr></table></td></tr></table><table cellpadding=0 cellspacing=2 width=$tablewidth align=center><tr height=4></tr><tr><td>$topicpages，<b>转至</b>：<input type=text name="dshow" size=2 value="$pn"> <input type=button value=Go onClick="Jumptopage(this.form)"><BR> [本版共有主题 <B>$threads</B> 篇，回复 <b>$posts</b> 篇]</td>$multimanageitem</form>~;
...
$output .= qq~<table cellpadding=1 cellspacing=0 width=$tablewidth align=center><tr><td align=right>$jumphtml</td></tr></table>~;
