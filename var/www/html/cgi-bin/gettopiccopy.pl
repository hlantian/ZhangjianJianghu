...
#####################################################
    ...
    $post =~ s/\<br\>/\n/ig;
     ...
     if (($post=~/LBHIDDEN\[(.*?)\]LBHIDDEN/sg)||($post=~/LBSALE\[(.*?)\]LBSALE/sg)) {
        unless (($inmembername eq $membername )||($mymembercode eq "ad") || ($mymembercode eq 'smo') || ($mymembercode eq "cmo") || ($mymembercode eq "amo") || ($mymembercode eq "mo") || ($inmembmod eq "yes")|| ($myrating >= $1)) {
    ...
    &error("复制帖子&对不起，您不允许复制被屏蔽的帖子！") if (($membercode eq "masked")&&($mymembercode ne "ad")&&($mymembercode ne 'smo')&&($mymembercode ne "cmo")&&($inmembmod ne "yes"));
    ...
    $temppost = qq~-=-=-=-=-=>\n$rawpost\n-=-=-=-=-=>~;
    $output .= qq~<table cellpadding=0 cellspacing=0 width=$tablewidth bgcolor=$tablebordercolor align=center><tr><td>
<table cellpadding=4 cellspacing=1 width=100%><tr><td bgcolor=$titlecolor colspan=2>
...
<tr><td bgcolor=$miscbackone colspan=2 align=center><input type="button" value="高亮显示文字" onClick="this.form.inpost.focus();this.form.inpost.select();">　　<input type="button" value="复制到剪贴板" name="cmdCopy" onClick="copy(this.form.inpost)"></center></td></tr></td></table></table>
    ~;
1;
