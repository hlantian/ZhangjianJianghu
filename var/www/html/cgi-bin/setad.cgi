...
var status = 0;
function ADShow(){
for (i=0;i<LeoBBSad.length;i++){
var addisplay=LeoBBSad[i];
if (status==0){
...
if (status==0){
status = 1;setTimeout("ADShow()",4000);
}
else{
status = 0;setTimeout("ADShow()",600);
}}
ADShow();
~;

  open(FILE,">${imagesdir}ad.js");
  print FILE $adsjscode;
  close(FILE);
 }
 else { unlink("${imagesdir}ad.js"); }
}
