...
if ($query -> param('getpassq')=~/\|/ || $query -> param('getpassa')=~/\|/) {
    &error("ÂÛÌ³ÃÜÂëÌáÊ¾ÎÊÌâºÍ´ğ°¸&ÂÛÌ³ÃÜÂëÌáÊ¾ÎÊÌâºÍ´ğ°¸ÖĞ£¬²»ÔÊĞíÓĞ·Ç·¨×Ö·û£¬Çë¸ü»»ÌáÎÊºÍ´ğ°¸£¡");
}
...
if ($regonoff eq 1) {
    ...
    &error("ÓÃ»§×¢²á&¶Ô²»Æğ£¬ÄúÊäÈëµÄÓÃ»§ÃûÓĞÎÊÌâ") if ($inmembername =~ /^q(.+?)-/ig);
    ...
    $bannedmember = "yes" if ($bannedips =~ /$ipaddresstemp/i);

    $bannedmember = "yes" if (($inmembername =~ /×¢²á/i)||($inmembername =~ /guest/i)||($inmembername =~ /qq-/i)||($inmembername =~ /q-/i)||($inmembername =~ /qx-/i)||($inmembername =~ /qw-/i)||($inmembername =~ /qr-/i)||($inmembername =~ /^È«Ìå/i)||($inmembername =~ /register/i)||($inmembername =~ /³ÏÆ¸ÖĞ/i)||($inmembername =~ /°ßÖñ/i)||($inmembername =~ /¹ÜÀíÏµÍ³Ñ¶Ï¢/i)||($inmembername =~ /leobbs/i)||($inmembername =~ /leoboard/i)||($inmembername =~ /À×°Á/i)||($inmembername =~ /LB5000/i)||($inmembername =~ /È«Ìå¹ÜÀíÈËÔ±/i)||($inmembername =~ /¹ÜÀíÔ±/i)||($inmembername =~ /ÒşÉí/i)||($inmembername =~ /¶ÌÏûÏ¢¹ã²¥/i)||($inmembername =~ /ÔİÊ±¿ÕÈ±/i)||($inmembername =~ /£ª£££¡£¦£ª/i)||($inmembername =~ /°æÖ÷/i)||($inmembername =~ /Ì³Ö÷/i)||($inmembername =~ /nodisplay/i)||($inmembername =~ /^system/i)||($inmembername =~ /---/i)||($inmembername eq "admin")||($inmembername eq "root")||($inmembername eq "copy")||($inmembername =~ /^sub/)||($inmembername =~ /^exec/)||($inmembername =~ /\@ARGV/i)||($inmembername =~ /^require/)||($inmembername =~ /^rename/i)||($inmembername =~ /^dir/i)||($inmembername =~ /^print/i)||($inmembername =~ /^con/i)||($inmembername =~ /^nul/i)||($inmembername =~ /^aux/i)||($inmembername =~ /^com/i)||($inmembername =~ /^lpt/i));

    if ($bannedmember eq "yes") { &error("ÓÃ»§×¢²á&²»ÔÊĞí×¢²á£¬ÄãÌîĞ´µÄÓÃ»§Ãû¡¢Email »òµ±Ç°µÄ IP ±»Ì³Ö÷ÉèÖÃ³É½ûÖ¹×¢²áĞÂÓÃ»§ÁË£¬Çë¸ü»»»òÕßÁªÏµÌ³Ö÷ÒÔ±ã½â¾ö£¡"); }
...
if ($addme) {
    ...
    my $check_result = &checkverify2("$ipaddress");
    if($check_result ne ""){
	if($check_result eq 1) { &error("ÓÃ»§×¢²á&ÑéÖ¤ÂëÊäÈë´íÎó£¡Çë·µ»ØÖØĞÂÌîĞ´£¡"); } else { &error("ÓÃ»§×¢²á&ÑéÖ¤Âë¹ıÆÚ£¡Çë·µ»ØË¢ĞÂÖØĞÂ»ñÈ¡ÑéÖ¤ÂëÒ»´Î£¡"); }
    }
...
if ($password ne "") {
	...
	&checkmemfile($recomm_q,$namenumber);
        ...
        if (-e $filetoopen) { &recommfunc("$recommender"); }
   ...
   if (open (MEMFILE, ">>$filetomakeopen")) {
	...
	$message .= "Ö÷  Ò³£º http://$homepage <br>\n";
    ...
    $output .= qq~<tr>
	<td bgcolor=$titlecolor $catbackpic valign=middle align=center><font color=$fontcolormisc><b>¸ĞĞ»Äú×¢²á£¬$inmembername</b></font></td></tr><tr>
...
<br><br><b>¹ØÓÚ×Ô¶¨ÒåÍ·Ïñ</b>£º<br>ÄãÒ²¿ÉÒÔÔÚÕâÀï¸ø³öÄã×Ô¶¨ÒåÍ·ÏñµÄ URL µØÖ·£¬Í·ÏñµÄ¸ß¶ÈºÍ¿í¶È(ÏñËØ)¡£ Èç¹û²»ÏëÒª×Ô¶¨ÒåÍ·Ïñ£¬Çë½«ÏàÓ¦À¸Ä¿È«²¿Áô¿Õ£¡<BR><BR>
...
<form action="$thisprog" method=post name="creator" enctype="multipart/form-data"><tr>
...
<tr><td bgcolor=$miscbacktwo><font color=$fontcolormisc><b>×¢²áÑéÖ¤Âë£º(ÑéÖ¤ÂëÓĞĞ§ÆÚÎª20·ÖÖÓ)</b><br>ÇëÊäÈëÓÒÁĞµÄÑéÖ¤Âë£¬ÊäÈë²»ÕıÈ·Ê±½«²»ÄÜÕı³£×¢²á¡£<br>£¨×¢Òâ£ºÖ»ÓĞÊı×Ö£¬ 0 ÊÇÁã¶ø²»ÊÇÓ¢ÎÄ×ÖÄ¸µÄ O£©</font></td><td bgcolor=$miscbacktwo><input type=text name="verifynum" size=4 maxlength=4> * <img src=verifynum.cgi height=24 align=absmiddle> Ò»¹²ÊÇËÄ¸öÊı×Ö£¬Èç¹û¿´²»Çå£¬ÇëË¢ĞÂ</td></tr>
<tr><td bgcolor=$miscbackone><font color=$fontcolormisc><b>ÍÆ¼öÈËÓÃ»§Ãû£º</b><br>ÊÇË­ÍÆ¼öÄú¼ÓÈëÎÒÃÇµÄÉçÇøµÄ£¿(Õâ½«Ê¹ÄãµÄÍÆ¼öÈË÷ÈÁ¦ÖµÔö³¤)</td>
...
sub recommfunc {
    ...
    $recommender = &stripMETA($recommender);
	...
	my $filedata = <REFILE>;
	chomp $filedata;
	($lmembername, $lpassword, $lmembertitle, $lmembercode, $lnumberofposts, $lemailaddress, $lshowemail, $lipaddress, $lhomepage, $loicqnumber, $licqnumber ,$llocation ,$linterests, $ljoineddate, $llastpostdate, $lsignature, $ltimedifference, $lprivateforums, $luseravatar, $luserflag, $luserxz, $lusersx, $lpersonalavatar, $lpersonalwidth, $lpersonalheight, $lrating, $llastgone, $lvisitno, $laddjy, $lmeili, $lmymoney, $lpostdel, $lsex, $leducation, $lmarry, $lwork, $lborn, $lchatlevel, $lchattime, $ljhmp, $ljhcount,$lebankdata,$lonlinetime,$luserquestion,$lawards,$luseradd7,$luserface,$lsoccerdata,$luseradd5) = split(/\t/,$filedata);
	...
	if (($lmembername ne "")&&($lpassword ne "")) {
	    seek(REFILE,0,0);
	    print REFILE "$lmembername\t$lpassword\t$lmembertitle\t$lmembercode\t$lnumberofposts\t$lemailaddress\t$lshowemail\t$lipaddress\t$lhomepage\t$loicqnumber\t$licqnumber\t$llocation\t$linterests\t$ljoineddate\t$llastpostdate\t$lsignature\t$ltimedifference\t$lprivateforums\t$luseravatar\t$luserflag\t$luserxz\t$lusersx\t$lpersonalavatar\t$lpersonalwidth\t$lpersonalheight\t$lrating\t$llastgone\t$lvisitno\t$laddjy\t$lmeili\t$lmymoney\t$lpostdel\t$lsex\t$leducation\t$lmarry\t$lwork\t$lborn\t$lchatlevel\t$lchattime\t$ljhmp\t$ljhcount\t$lebankdata\t$lonlinetime\t$luserquestion\t$lawards\t$luseradd7\t$luserface\t$lsoccerdata\t$luseradd5\t";
	    close(REFILE);
   	} else {
	    close(REFILE);
	}
...
sub checkverify2 {
    my $trueipaddress = shift;
    my $verifynum = $query -> param('verifynum');
    mkdir ("${lbdir}verifynum", 0777) unless (-e "${lbdir}verifynum");
    mkdir ("${lbdir}verifynum/login", 0777) unless (-e "${lbdir}verifynum/login");

###»ñÈ¡µ±Ç°½ø³ÌµÄÑéÖ¤ÂëºÍÑéÖ¤Âë²úÉúÊ±¼ä¡¢ÓÃ»§ÃÜÂë
my $filetoopen = "${lbdir}verifynum/$trueipaddress.cgi";
open(FILE, $filetoopen);
my $content = <FILE>;
close(FILE);
chomp($content);
...
if ($verifynum ne $trueverifynum || $currenttime > $verifytime + 1200+120) { #ÑéÖ¤ÂëÓĞĞ§Ê±¼äÎª 15 ·ÖÖÓ + 2 ·ÖÖÓÔ¤Áô
    if ($currenttime > $verifytime + 900) { unlink($filetoopen); return 2;} else { return 1; }
} else {
    unlink($filetoopen);
    return '';
}
