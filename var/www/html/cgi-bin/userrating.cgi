...
$output .= qq~
<br>
<table width=$tablewidth align=center cellspacing=0 cellpadding=1 bgcolor=$navborder><caption align=left>>>> 在这里您可以对用户进行投票，减少或增加他们的威望，甚至可以禁止他们发言！</caption><tr><td><table width=100% cellspacing=0 cellpadding=3 height=25><tr><td bgcolor=$navbackground><img src=$imagesurl/images/item.gif align=absmiddle width=11> <font face="$font" color=$navfontcolor> <a href="leobbs.cgi">$boardname</a> → <a href="forums.cgi?forum=$inforum">$forumname</a> → 给用户投票<td bgcolor=$navbackground align=right></td></tr></table></td></tr></table>
<p>
<p>
...
<tr><td bgcolor=$titlecolor $catbackpic colSpan=2 align=center><font color=$fontcolormisc><b>请首先登录然后对 $editmembername 投票(仅对坛主和版主开放)</b></font></td></tr>
...
elsif ($action eq "logmein")
{
	...
	&error("用户投票&没有该注册用户") if ($userregistered eq "no");
	&error("给用户投票&坛主和版主不能被投票") if ($membercode eq "ad" || $membercode eq "smo" || $membercode eq "cmo" || $membercode eq "amo" || $membercode eq "mo");
	...
	elsif ($rating == -5) {$pwout = qq~<input type=radio name=pw value="praise" checked>赞扬用户(威望加 1)　　<input type=radio name=pw value="warn">禁止用户发言(威望减为 -6)　　<input type=radio name=pw value="worstm">屏蔽此用户贴子(威望减为 -6)</td>~;}
	elsif ($rating == -6) {$pwout = qq~<input type=radio name=pw value="praise" checked>恢复用户(威望恢复成 -5)~;}
	else {$pwout = qq~<input type=radio name=pw value="praise" checked>赞扬用户(威望加 1)　　<input type=radio name=pw value="warn">警告用户(威望减 1)　　<input type=radio name=pw value="reset">清零(威望为 0)　　<input type=radio name=pw value="worst">禁止发言(威望减为 -6)　　<input type=radio name=pw value="worstm">屏蔽此用户贴子(威望减为 -6)</td>~;}

	$output .= qq~
<tr><td bgcolor=$titlecolor $catbackpic align=center><font color=$fontcolormisc><b>$editmembername 目前的威望是: $rating</b></font></td></tr>
...
elsif ($action eq "process")
{
	...
	$trueipaddress = "no" if ($trueipaddress eq "" || $trueipaddress eq "unknown" || $trueipaddress =~ m/^192\.168\./ || $trueipaddress =~ m/^10\./);
	my $thistime = time;
	...
	&addadminlog("对用户 $editmembername 投票威望 $oldrating → $rating，理由：$reason", $intopic);
	...
	if ($msgnotify eq "yes") {
	    $topictitle =~ s/^＊＃！＆＊//;
	    &shortmessage($inmembername, $editmembername, "你已经被$pwmail!", "　　你已经被 $inmembername $pwmailing! 你现在的威望是: $rating。<br>　　相关的主题是: <a href=topic.cgi?forum=$inforum&topic=$intopic target=_blank>$topictitle</a>，操作原因是: $reason。");
	}
	$output .= qq~
