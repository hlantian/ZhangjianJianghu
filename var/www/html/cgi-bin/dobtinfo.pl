...
if ($dispshowcount eq "yes") {
    ...
    $addme = qq~<a href="javascript:Download('$forumno','$topicno','$replyno')"><img src=$imagesurl/icon/$filetype.gif border=0 width=16><a> <a href="javascript:Download('$forumno','$topicno','$replyno')">该主题有一个 BitTorrent 格式的文件，按此下载</a> (共 $fileinfo[7] 字节)$DownTime<br>~;
} else {
...
foreach (@btfileinfo) {
    ...
    $addme .= qq~<tr bgColor=$postbackcolor $addme1><td align=middle nowrap><font color=$postfontcolor>$filename</td><td align=middle nowrap><font color=$postfontcolor>$filelength</td></tr>~;
}
if ($count >= 8 ) { $addme .= qq~<tr bgColor=$postbackcolor id=BFILE style=display:""><td align=right nowrap colspan=2><span style=CURSOR:hand onclick=ShowMore()><font color=$postfontcolor title=显示所有文件>更多...</font></span>&nbsp;</td></tr>~; }
