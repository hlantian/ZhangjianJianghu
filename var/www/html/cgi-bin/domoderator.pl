...
<img src=$imagesurl/images/team2.gif width=19align=absmiddle><select OnChange="surfto(this);"><option style="color: $fonthighlight">本版管理员列表</option>~ if ($modnumber > 0 || $modnumber1 > 0 || $cmodnumber > 0);
        if ($cmodnumber > 0) {
        ...
        $inmembmod = "yes" if ($forummodnamestemp =~ /\,$inmembername\,/i || (($membercode eq "cmo" || $membercode eq "mo" || $membercode eq "amo") && ($forummodnamestemp =~ /,全体版主,/ || $forummodnamestemp =~ /,全体斑竹,/)));
		...
		#     $topictitletemp =~ s/\&/\&amp;/g;
		    ...
		    if ($teamlogo eq "") { $team=""; }
	    		elsif (($teamurl eq "")||($teamurl eq "http://")) { $team=qq~$teamlogo~; }
