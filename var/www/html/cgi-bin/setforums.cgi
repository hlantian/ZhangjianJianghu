...
<font color=#333333><b>图像预览(不支持 Flash)</b></font><br><IMG border=0 name=bbsimg src="$imagesurl/myimages/blank.gif" align="absmiddle" onload="javascript:if(this.width>screen.width-333)this.width=screen.width-333"></td>
        </tr>
        <tr>
...
foreach $_ (@molist){
    ...
    if ((!(-e "${lbdir}$memdir/$namenumber/$_.cgi"))&&(!(-e "${lbdir}$memdir/old/$_.cgi"))) { &winunlock($filetoopen) if ($OS_USED eq "Nt"); &errorout("论坛版主名单中，$_ 这个用户 ID 是不存在的！");}
                ...
                print FILE "$newforumid\t$new_categoryname\t$new_categorynumber\t$new_forumname\t$new_forumdescription\t$new_forummoderator\t$new_htmlstate\t$new_idmbcodestate\t$new_privateforum\t$new_startnewthreads\t\t\t0\t0\t$new_forumgraphic\t$new_ratings\t$misc\t$new_forumpass\t$new_hiddenforum\t$new_indexforum\t$new_teamlogo\t$new_teamurl\t$new_fgwidth|$new_fgwidth1\t$new_fgheight|$new_fgheight\t$new_miscad4\t$todayforumpost\t$new_miscad5\t";
                close(FILE);
...
#	        flock(FILE, 1) if ($OS_USED eq "Unix");
                ...
                print FILE "$newforumid\t$new_categoryname\t$new_categorynumber\t$new_forumname\t$new_forumdescription\t$new_forummoderator\t$new_htmlstate\t$new_idmbcodestate\t$new_privateforum\t$new_startnewthreads\t\t\t0\t0\t$new_forumgraphic\t$new_ratings\t$misc\t$new_forumpass\t$new_hiddenforum\t$new_indexforum\t$new_teamlogo\t$new_teamurl\t$new_fgwidth|$new_fgwidth\t$new_fgheight|$new_fgheight\t$new_miscad4\t$todayforumpost\t$new_miscad5\t\n" if($new_categoryname eq "childforum-$tempforumno");
                $Get=1;
                    }
                    }
         ...
         foreach $forum (@forums) {
 	     ...
 	     next if ($forum eq "");
             ...
             if ($forumid eq $inforum) {
                ...
                $editedline = "$inforum\t$category\t$categoryplace\t$new_forumname\t$new_forumdescription\t$new_forummoderator\t$new_htmlstate\t$new_idmbcodestate\t$new_privateforum\t$new_startnewthreads\t$lastposter\t$lastposttime\t$threads\t$posts\t$new_forumgraphic\t$new_ratings\t$misc\t$new_forumpass\t$new_hiddenforum\t$new_indexforum\t$new_teamlogo\t$new_teamurl\t$new_fgwidth|$new_fgwidth\t$new_fgheight|$new_fgheight\t$new_miscad4\t$todayforumpost\t$new_miscad5\t";
                chomp $editedline;
