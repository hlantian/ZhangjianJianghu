...
for ('inmembername','inpassword','action','CookieDate','onlineview','advpost','viewMode','nodispavatar',
...
~;
	...
	$nodisp            = cookie(-name => "nodisp",            -value => "$nodispavatar|$nodispsign|$nodispphoto", -path => "$cookiepath/", -expires => "$CookieDate");
  	...
  	print header(-cookie=>[$onlineviewcookie,$advpostcookie,$threadcookie,$viewcookie, $nodisp, $freshtimecookie, $selectstylecookie, $tanchumsgcookie, $namecookie,$passcookie] , -expires=>"$EXP_MODE" , -cache=>"$CACHE_MODES");
...
~;
	...
	$tecookie	   = cookie(-name => "catlog"           , -value => "", -path => "$cookiepath/");
        ...
        print header(-cookie=>[$banfreshcookie, $treeviewcookie, $tecookie, $namecookie, $passcookie, $trashcookie,$onlineviewcookie,$advpostcookie,$viewcookie, $nodisp, $freshtimecookie, $selectstylecookie, $tanchumsgcookie] , -expires=>"$EXP_MODE" , -cache=>"$CACHE_MODES");
...
<tr><td bgcolor=$miscbackone><font color=$fontcolormisc>请输入您的密码</font></td><td bgcolor=$miscbackone><input type=password name="inpassword"> &nbsp; <font color=$fontcolormisc><a href="profile.cgi?action=lostpass" style="cursor:help">忘记密码？</a></font></td></tr>
    ...
    <td bgcolor=$miscbackone valign=middle><font class='misc'>
                <input type="radio" name="CookieDate" value="-1d" checked id=3> <label for=3>不保存，关闭浏览器就失效</label> <font color=$fonthighlight>(在网吧等公共场所建议选择这个)</font><br>
