...
#####################################################
	...
	$checked=$1;
	if (($inmembername eq $membername )||($mymembercode eq "ad") || ($mymembercode eq 'smo') || ($mymembercode eq "mo") || ($mymembercode eq "amo") || ($inmembmod eq "yes")|| ($myrating >= $1) ){
    ...
    if ($advpost == 1) { $advpostmt="<B>[<a href=$thisprog?action=replyquote&forum=$inforum&topic=$intopic&postno=$inpostno&advpost=0><font color=$fonthighlight>简洁模式</font></a>]</B>"; }
                  else { $advpostmt="<B>[<a href=$thisprog?action=replyquote&forum=$inforum&topic=$intopic&postno=$inpostno&advpost=1><font color=$fonthighlight>高级模式</font></a>]</B>"; $insidejs = ""; }
    $output .= qq~<script>
var autoSave = false;
...
<font color=$titlefontcolor><b>主题标题</b>： $topictitle　$advpostmt</td></tr>
~;
    &posttable(2);
