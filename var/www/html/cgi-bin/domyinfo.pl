...
#    $jingyan = $numberofposts * $ttojy + $numberofreplys * $rtojy + $visitno * $ltojy + $jingyan - $postdel * $deljingyan + $jhcount * $addjhjy;
    ...
    my $myoutput = qq~<tr><td bgcolor=$titlecolor colspan=4 $catbackpic><font color=$titlefontcolor><B>-=>  您的个人状态</B>  [ IP 是：$trueipaddress，来自：$fromwhere1 。操作系统：$osinfo，浏览器：$browseinfo ]</td></tr>
...
<td bgcolor=$forumcolortwo width=* valign=top><img src=$imagesurl/images/none.gif width=340 height=1><BR>
...
[<a href=messanger.cgi?action=inbox target=_blank>收件箱</a>]信息数: <B>totalmessageshtc</B> 条<BR>
[<a href=messanger.cgi?action=outbox target=_blank>发件箱</a>]信息数: <B>$totalmessagesout</B> 条<BR>
短消息条数限制: $maxmsgno<BR>
</td>
<td nowarp>
