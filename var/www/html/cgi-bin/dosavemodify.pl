...
#####################################################
    ...
    if ($query -> param('getpassq')=~/\|/||$query -> param('getpassa')=~/\|/) {
        &error("论坛密码提示问题和答案&论坛密码提示问题和答案中，不允许有非法字符，请更换提问和答案！");
    }
...
#       if ($newpassword1 =~ /^[0-9]+$/) { &error("用户注册&论坛密码请不要全部为数字，请更换！"); }
    ...
    if($newemailaddress !~ /^.+\@(\[?)[a-zA-Z0-9\-\.]+\.([a-zA-Z]{2,3}|[0-9]{1,3})(\]?)$/) { &error("注册&Email 地址非法$newemailaddress！"); }
...
#    &getmember("$inmembername","no");
	...
	$passcookie = cookie(-name => "apasswordcookie", -value => "", -path => "$cookiepath/", -expires => "-1d");
    ...
    if (($editusertitleself eq "post") && (($numberofposts + $numberofreplys) >= $needpoststitle)) { $editusertitleself = "on"; }
	...
	if (open(FILE, "+<$filetomake")) {
	    ...
	    seek(FILE,0,0);
	    print FILE "$inmembername\t$newpassword\t$membertitle\t$membercode\t$numberofposts|$numberofreplys\t$newemailaddress\t$newshowemail\t$ipaddress\t$newhomepage\t$newoicqnumber\t$newicqnumber\t$newlocation\t$newinterests\t$joineddate\t$lastpostdate\t$newsignature\t$newtimedifference\t$privateforums\t$inuseravatar\t$inuserflag\t$inuserxz\t$inusersx\t$newpersonalavatar\t$newpersonalwidth\t$newpersonalheight\t$rating\t$lastgone\t$visitno\t$addjy\t$meili\t$mymoney\t$postdel\t$newsex\t$neweducation\t$newmarry\t$newwork\t$newborn\t$chatlevel\t$chattime\t$jhmp\t$jhcount\t$ebankdata\t$onlinetime\t$userquestion\t$awards\t$useradd7\t$userface\t$soccerdata\t$useradd5\t";
	    close(FILE);
	}
