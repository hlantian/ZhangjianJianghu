...
public class ChatCommand implements CommandHandler {
    ...
    public String getDescription() {
        return "在公共频道发言 (chat 消息)";
    }

    @Override
    public void handle(GameEngine engine, Player player, String args) {
        if (player.getCloseChat() != null && player.getCloseChat() == 1) {
            engine.getMessageService().sendToPlayer(player.getId(), "你的公共频道是关闭的。");
            return;
        }
